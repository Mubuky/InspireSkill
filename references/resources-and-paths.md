...
Compute Group 名称里的资源区前缀也是调度语义。qz 当前用 `小卡区` 承载单节点 4 GPU 及以下任务，用 `整卡区` 承载单节点 8 GPU 或 8 GPU 倍数任务；具体可用型号、机房和三元组仍以当前 Workload 的 Live Quota Row 为准。创建 Workload 或写 Profile 时，从同一行复制完整 `group` 和 `quota`，不要把小卡区的三元组配到整卡区名称，或反过来。多节点 Job 的 `--nodes` 是实例数 / 节点数放大，不改变每个节点所选 `quota` 的分区语义。
