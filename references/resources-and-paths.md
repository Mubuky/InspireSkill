...
Compute Group 名称里的资源区前缀也是调度语义，例如 `开发区` / `训练区`。开发区同时支持整节点和碎卡任务；训练区优先保障整节点任务，碎卡任务仅允许低优先级。具体可用型号、机房和三元组仍以当前 Workload 的 Live Quota Row 为准；创建 Workload 或写 Profile 时从同一行复制完整 `group` 和 `quota`。
