...
### Restricted Notebook File Movement

Restricted notebooks do not use SSH/SCP/rsync. Use shared storage as the boundary:

1. Pick a public-internet notebook in the same account/project context.
2. Use `inspire notebook scp <public-notebook> ... /inspire/<storage>/...` to upload or download shared-path files. If you need rsync semantics, generate or use SSH config for the public-internet notebook and run external `rsync` against that notebook with the same `/inspire/...` path.
3. Use `inspire notebook exec <restricted-notebook> "..."` or `inspire notebook shell <restricted-notebook>` to operate on that same `/inspire/<storage>/...` path inside the restricted notebook.

Examples:

```bash
# Upload from the local machine into shared storage through a public-internet notebook.
inspire notebook scp cpu-box ./dataset.tar /inspire/hdd/project/topic/user/dataset.tar

# Use it from the restricted notebook without SSH.
inspire notebook exec gpu-box "ls -lh /inspire/hdd/project/topic/user/dataset.tar"

# Download a shared-path artifact through the public-internet notebook.
inspire notebook scp cpu-box -d /inspire/hdd/project/topic/user/results.tar ./results.tar

# Same idea with external rsync after configuring SSH for the public-internet notebook.
rsync -av ./dataset/ cpu-box:/inspire/hdd/project/topic/user/dataset/
```

Container-local paths outside `/inspire/...` on restricted notebooks are not directly transferable by the local CLI. Put artifacts under `/inspire/...` first if they need to leave the restricted notebook.
