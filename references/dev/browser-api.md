...
本文档只收录已经闭合的 Browser API 合同：当前仓库必须有 wrapper、底层 helper 或用户命令引用，请求体、主要响应字段和 Referer 来源必须能从当前代码、测试、live smoke、前端抓包或 SPA bundle 中对齐。只观察到路径存在、字段不完整或 destructive 语义没有验证的内容，不进入端点清单。

读表规则：

- `CLI 引用` 列必须指向当前仓库中的 wrapper、helper 或用户命令；没有当前引用就不列。
