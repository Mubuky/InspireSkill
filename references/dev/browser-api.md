...
| `POST` | `{prefix}/project/list_v2` | 当前前端表单通用项目下拉。body 实测两种：`{page:1,page_size:-1,filter:{workspace_id,check_admin:true}}` 和 `{page:1,page_size:-1,filter:{workspace_id}}`；返回 `{items,total}` | 未封装；Chrome reverse capture（2026-05-09）确认，现有 CLI 仍用 `/project/list` |
| `POST` | `{prefix}/project/list_for_page` | 项目管理页分页列表。body: `{page:1,page_size:10,filter:{}}`；返回 `{items,total}`，项目条目包含预算、维护者、`space_list` 等完整管理视图字段 | 未封装；Chrome reverse capture（2026-05-09）确认 |
...
| `POST` | `{prefix}/notebook/users` | notebook 页面用户筛选下拉。body: `{workspace_id}`；返回 `{list,total}`，单条为用户 metadata | 未封装；Chrome reverse capture（2026-05-09）确认，CLI 当前用当前用户详情或 notebook 列表字段解析 |
...
| `POST` | `{prefix}/lifecycle/list` | notebook 生命周期状态转换记录。body: `{notebook_id, page, page_size, start_time, end_time}`。**实测在 2026-04 的平台上对普通 notebook 经常返回 `{list:[], total:0}`** —— 网页的"生命周期"tab 实际是靠 `/run_index/list` 画的；CLI 保留 thin wrapper，主要展示面走 `run_index/list` | `browser_api.notebooks.list_notebook_lifecycle`（thin wrapper） |
...
| `POST` | `{prefix}/train_job/instance_list` | 该任务的 pod 实例。body: `{jobId, page_num, page_size}` | `browser_api.jobs.list_job_instances`；`inspire job instances` / `job shell` / `job logs` |
...
| `POST` | `{prefix}/hpc_jobs/instances/list` | HPC pod / component 实例列表。body: `{jobId, page_num, page_size}`；返回 `{items,total}`，单条含 `component`、`name`、`node`、`status`、`created_at`、`started_at`、`finished_at`、`running_time_ms` | 未封装；2026-05-09 用历史 `SUCCEEDED` HPC job live probe 确认 |
| `POST` | `{prefix}/logs/hpc` | HPC 聚合日志。body: `{page_size, filter:{podNames:[...], start_timestamp_ms:"...", end_timestamp_ms:"..."}}`；时间戳同 train logs 用字符串 epoch-ms。不要传 `sorter:[{field:"@timestamp"...}]`，实测返回 `code:1600003`，消息为日志排序字段不合法 | 未封装；2026-05-09 用真实 HPC 实例名 live probe 确认 |
...
当前封装覆盖 `inspire model list/status/versions/register`。本文档只描述下表列出的模型仓库合同；未列入的页面功能不属于当前 CLI 合同。当前前端入口是 `/jobs/modelService?spaceId=<workspace_id>`。
...
网页端部署表单已实测确认：当前 CLI 合同覆盖“自定义部署”。自定义部署使用注册模型、镜像 `mirror_id`、运行命令、端口、优先级、资源规格、副本数、单副本实例数和共享内存。`resources specs --usage serving` 必须查 `SCHEDULE_CONFIG_TYPE_SERVE`，不是训练任务的 `SCHEDULE_CONFIG_TYPE_TRAIN`。在 `H200-2号机房` 实测到的 serving 规格是 `1,18,200`、`2,36,400`、`4,72,900`、`8,168,1800`。
