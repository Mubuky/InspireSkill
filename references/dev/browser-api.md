...
日常默认看人类输出；结构化输出只用于脚本。平台的 `list` 端点会把最新版本属性合入模型条目，而 `detail` 端点只返回模型主记录，所以 `is_vllm_compatible` 在 `list` 与 `status` 中可能不一致。要准确的版本属性，用 `inspire model versions <model-id>`。
