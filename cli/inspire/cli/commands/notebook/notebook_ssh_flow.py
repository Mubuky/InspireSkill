    ...
    def _attempt_rebuild() -> bool:
        ...
        if reconnect_result.status is NotebookBridgeReconnectStatus.NOT_REBUILDABLE:
            _handle_error(
                ...
                EXIT_CONFIG_ERROR,
                hint="Re-run 'inspire notebook ssh connect <notebook>' to re-bootstrap.",
            )
            return False
        ...
        _handle_error(
            ...
            EXIT_API_ERROR,
            hint="Re-run 'inspire notebook ssh connect <notebook>' to refresh the tunnel.",
        )
        return False
    ...
    *,
    ...
    if not is_tunnel_available(
        ...
        _handle_error(
            ...
            EXIT_API_ERROR,
            hint=(
                f"Retry 'inspire notebook ssh connect {scrub_raw_ids(profile_name)}' in a few seconds, "
