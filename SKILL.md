...
Compute Group 名称可能承载平台调度区语义，例如 qz 的 `小卡区` / `整卡区`；选择资源时以同一条 Live Quota Row 同时确定 `group` 和 `quota`，Profile 也从同一行复制这两个值，不要把不同 Row 的名称和三元组拼接使用。`quota` 是单实例 / 单节点规格，Job 的 `--nodes` 只放大节点数，不改变小卡区 / 整卡区的单节点语义。
